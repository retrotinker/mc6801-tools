...
extern void
extract_members(void (*function) (struct member_desc member, FILE * istream));
extern void scan(void (*function) (struct member_desc member, FILE * istream),
		 int crflag);
...
extern int read_header_info(struct mapelt *mapelt, int desc, long int offset,
			    long int *syms_offset, unsigned int *syms_size,
			    long int *strs_offset, unsigned int *strs_size);
extern struct mapelt *find_mapelt(struct mapelt *map, char *name);
extern struct mapelt *find_mapelt_noerror(struct mapelt *map,
					  register char *name);
...
extern void copy_out_member(struct mapelt *mapelt, int archive_indesc,
			    int outdesc, char *outname);
...
extern void write_symdef_member(struct mapelt *mapelt, struct mapelt *map,
				int outdesc, char *outname);
#endif

...
void mywrite(desc, pbuf, bytes, file)
int desc;
void *pbuf;
int bytes;
char *file;
...
int crflag;
{
	...
	/* Now find the members one by one.  */
	{
		...
		while (1) {
			...
			nread =
			    fread(&member_header, 1, sizeof(struct ar_hdr),
				  arcstream);
			if (nread == 0)
			...
			if (nread != sizeof(member_header)
				...
				fatal("file %s not a valid archive", archive);
			bcopy(member_header.ar_name, name,
			...
			member_desc.data_offset =
			    member_offset + sizeof(member_header);
			...
			member_offset +=
			    sizeof(member_header) + member_desc.size;
			if (member_offset & 1)
				++member_offset;
		}
	}
...

void print_descr(member, instream)
...
FILE *instream;
{
	...
	printf(" %2d/%2d %6d %12.12s %4.4s %s\n",
	       ...
	       member.size, timestring + 4, timestring + 20, member.name);
}

void print_modes(modes)
int modes;
{
...
void extract_member(member, istream)
struct member_desc member;
FILE *istream;
{
	int ncopied = 0;
...

void print_contents(member, istream)
struct member_desc member;
FILE *istream;
{
	int ncopied = 0;
...
struct mapelt *make_map(nonexistent_ok)
int nonexistent_ok;
{
...
void add_to_map(member, istream)
struct member_desc member;
FILE *istream;
{
...
struct mapelt *prev_mapelt(map, elt)
...
struct mapelt *find_mapelt_noerror(map, name)
struct mapelt *map;
register char *name;
{
...
struct mapelt *find_mapelt(map, name)
struct mapelt *map;
char *name;
{
...
void write_archive(map, appendflag)
struct mapelt *map;
int appendflag;
{
		...
		for (tail = map; tail != 0; tail = tail->next)
			if (tail->info.offset == 0) {
				...
				for (i = 0; i < tail->info.nsymdefs; ++i)
					tail->info.symdefs[i].offset =
					    tail->info.new_offset;
				doneany = 1;
			}
		if (doneany) {
...
void header_from_map(header, mapelt)
...
void copy_out_member(mapelt, archive_indesc, outdesc, outname)
struct mapelt *mapelt;
int archive_indesc;
int outdesc;
char *outname;
...
void touch_symdef_member(outdesc, outname)
int outdesc;
char *outname;
{
	struct stat statbuf;
...
void move_members()
{
	...
	case POS_BEFORE:
		after_mapelt =
		    prev_mapelt(change_map, find_mapelt(map, posname));
	}
...
int move_in_map(name, map, after)
char *name;
...
int insert_in_map(name, map, after)
char *name;
struct mapelt *map, *after;
{
...
#endif
{
	...
	if (!arcstream)
		fatal("failure opening archive %s for the second time",
		      archive);
	map = make_map(0);
...
void write_symdef_member(mapelt, map, outdesc, outname)
struct mapelt *mapelt;
struct mapelt *map;
int outdesc;
char *outname;
...
#ifdef HOST_TARGET_ENDIANESS_DIFFERS
			md_symdefs_to_chars((struct md_symdef *)mapptr->info.
					    symdefs, mapptr->info.nsymdefs,
					    (unsigned char *)mapptr->info.
					    symdefs);
#endif
			write(outdesc, mapptr->info.symdefs,
			      mapptr->info.nsymdefs * sizeof(struct symdef));
#ifdef HOST_TARGET_ENDIANESS_DIFFERS
			md_chars_to_symdefs((unsigned char *)mapptr->info.
					    symdefs,
					    (struct md_symdef *)mapptr->info.
					    symdefs, mapptr->info.nsymdefs);
#endif
		}
...
#ifdef HOST_TARGET_ENDIANESS_DIFFERS
	new_strings_size =
	    md_chars_to_number((unsigned char *)&new_strings_size,
...

void read_old_symdefs(map, archive_indesc)
struct mapelt *map;
int archive_indesc;
{
	struct mapelt *mapelt;
	...
	if (len == sizeof hdr && !N_BADMAG(hdr)) {
		...
		*strs_offset = N_STROFF(hdr);
		...
		if (read(desc, (char *)strs_size, sizeof *strs_size) !=
		    sizeof *strs_size) {
			error_with_file("failure reading string table size in ",
					mapelt);
			return 0;
		}
#ifdef HOST_TARGET_ENDIANESS_DIFFERS
...
void make_new_symdefs(mapelt, archive_indesc)
struct mapelt *mapelt;
int archive_indesc;
{
	int indesc;
	...
	if (!read_header_info
	    (mapelt, indesc, offset, &syms_offset, &syms_size, &strs_offset,
	     &strs_size)) {
		if (mapelt->info.offset == 0)
			close(indesc);
		return;
	...
	for (i = 0; i < symcount; ++i) {
		unsigned long int stroff =
		    symbols[i].n_un.n_strx - sizeof(strs_size);
		char *symname = strings + stroff;
		if (stroff > strs_size) {
			char buf[100];
			sprintf(buf,
				"ridiculous string offset %lu in symbol %u of ",
...
int filter_symbols(syms, symcount)
struct nlist *syms;
...
void update_symdefs(map, archive_indesc)
struct mapelt *map;
int archive_indesc;
{
	struct mapelt *tail;
	...
	for (i = 0; i < num_old_symdefs; i++) {
		strcpy(new_strings + pos,
		       old_strings + old_symdefs[i].s.stringoffset);
...
void error_with_file(string, mapelt)
char *string;
struct mapelt *mapelt;
{
	fprintf(stderr, "%s: ", program_name);
