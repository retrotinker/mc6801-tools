...
char *where;
{
	return build_1hex_number((opcode_pt) num,
				 build_1hex_number((opcode_pt) (num / 256),
						   where));
}
...
PRIVATE void listcode()
{
	...
	if (popflags & POPLO) {
#if SIZEOF_OFFSET_T > 2
		if (popflags & POPLONG)
			build_2hex_number((u16_T)
					  (lastexp.offset /
					   (offset_t) 0x10000L),
					  listptr->displ4);
#endif
		if (popflags & POPHI)
			build_2hex_number((u16_T) lastexp.offset,
					  listptr->displ2);
		else
			build_1hex_number((opcode_pt) /* XXX */ (u16_T) lastexp.
					  offset, listptr->displ1);
		...
		if (count > 0) {
			build_1hex_number((opcode_pt) lastexp.offset,
					  listptr->displ1);
			if (lastexp.data & RELBIT)
				listptr->reldispl[0] = '>';
		}
...
PRIVATE void listerrors()
{
	...
	linep = linebuf;
	do {
		while (errptr && errptr->position < 132
		       && column < errptr->position) {
			++column;
