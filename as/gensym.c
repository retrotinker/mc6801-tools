...
PUBLIC void gensym()
{
	...
	if ((outfd = symfil) != 0) {
		...
		{
			int sft;
			if ((symptr = *copyptr++) == NUL_PTR)
				continue;
			if (globals_only_in_obj && !(symptr->type & EXPBIT))
				continue;
		...
		for (copyptr = symlptr; copyptr < copytop;) {
			...
			writec(symptr->type);
			write(outfd, symptr->name,
			      (unsigned)(symptr->length - 1));
				...
				if ((bool_t) nameflag) {
					if (strcmp
					    ((*left)->name,
					     (*right)->name) <= 0)
						break;
				} else if ((unsigned)(*left)->value_reg_or_op.
					   value <=
