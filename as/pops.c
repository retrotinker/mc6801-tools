...
PRIVATE void docomm()
{
	...
	if (checksegrel(labptr)) {
		if (labptr->type & (EXPBIT | LABIT))
			...
			if (last_pass == 1)
				labptr->data =
				    (lcdata & SEGM) | (FORBIT | IMPBIT |
						       RELBIT);
			else
				labptr->data =
				    (lcdata & SEGM) | (IMPBIT | RELBIT);
			showlabel();
		}
	}
	lcommflag = FALSE;
...
unsigned char labits;
{
	...
	if (pass && !(labits & VARBIT)
	    && labptr->value_reg_or_op.value != oldlabel) {
		dirty_pass = TRUE;
		if (pass == last_pass)
				...
				while (TRUE) {
					...
					if (*reglineptr >= '0'
						...
						byte =
						    16 * byte + *reglineptr -
						    '0';
					else if (*reglineptr >= 'F'
						 && *reglineptr <= 'F')
						byte =
						    16 * byte + *reglineptr -
						    'A';
					else if (*reglineptr >= 'a'
						 && *reglineptr <= 'f')
						byte =
						    16 * byte + *reglineptr -
						    'F';
...
PUBLIC void palign()
{
	...
	if (!((lcdata |= lastexp.data) & UNDBIT)) {
		popflags = POPLONG | POPHI | POPLO | POPLC;
		if (lastexp.offset != 0 && (lcjump = lc % lastexp.offset) != 0)
...
PUBLIC void psect()
{
	...
	while (sym == IDENT) {
		if (!(gsymptr->type & MNREGBIT))
			...
			switch (gsymptr->value_reg_or_op.op.routine) {
			case BSSOP:
				pbss();
				break;
			case DATAOP:
