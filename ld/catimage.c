...
char **argv;
{
	...
	for (image_id = 0; image_id < argc - 2; image_id++) {
		...
		if (header.a_flags & A_SEP) {
			...
			copy_segment(image_offset, A_DATAPOS(header),
				     header.a_data);
		} else {
			copy_segment(image_offset, A_TEXTPOS(header),
...
read_symtable()
{
	...
	while (fread(&item, sizeof(struct nlist), 1, ifd) == 1) {
		...
		/* Match the name */
		if (memcmp(item.n_name, "__seg", 5) != 0
		    || item.n_name[6] != '_')
			continue;
		...
		case 'd':
			data_offt[item.n_name[5] - '0'] =
			    base_off + item.n_value;
			break;
		case 't':
			text_offt[item.n_name[5] - '0'] =
			    base_off + item.n_value;
			break;
		}
		...
		case C_STAT:
			...
			break;
		default:
			printf("%-6d ", (item.n_sclass & N_CLASS));
			break;
		}
