...
#endif
{
	...
	if ((fd = fopen(argv[libarg], "wb")) == 0)
		fatalerror("Cannot open archive");
	if (fwrite(ARMAG, 1, SARMAG, fd) != SARMAG)
		fatalerror("Cannot write magic");

	for (ar = 1; ar < argc; ar++)
		if (ar != libarg && argv[ar][0] != '-') {
			char *ptr;
			if (stat(argv[ar], &st) < 0)
				fatalerror("Cannot stat object");
			if ((ptr = strchr(argv[ar], '/')))
				ptr++;
			else
				ptr = argv[ar];
			...
			sprintf(arbuf.ar_uid, "%-6d",
				(int)(st.st_uid % 1000000L));
			sprintf(arbuf.ar_gid, "%-6d",
			...
			if (fwrite(&arbuf, 1, sizeof(arbuf), fd) !=
			    sizeof(arbuf))
