...
bool_pt trace;
{
	...
	default:
		...
		while ((filelength = readarheader(&archentry)) > 0) {
			...
			if (magic == OMAGIC) {
				seekin(filepos);
				for (modcount = readfileheader();
				     modcount-- != 0;) {
					readmodule(stralloc(filename),
						   archentry);
	...
	for (endsymdptr = symdptr + nsymbol; symdptr < endsymdptr;
		...
		if ((flags = symdptr->dflags) & (E_MASK | I_MASK) &&
			...
			if ((symptr->flags & SEGM_MASK) == SEGM_MASK)
				symptr->flags &=
				    ~(flags_t) SEGM_MASK | (flags & SEGM_MASK);
			else if ((flags & SEGM_MASK) == SEGM_MASK)
				flags &=
				    ~(flags_t) SEGM_MASK | (symptr->
							    flags & SEGM_MASK);
			if ((flags ^ symptr->flags) & (A_MASK | SEGM_MASK)) {
				redefined(symname,
					  " with different segment or relocatability",
			...
			if (symptr->flags & E_MASK) {
				if (flags & E_MASK
				    && redsym(symptr, symdptr->dvalue))
					redefined(symname, "", archentry,
						  symptr->modptr->filename,
						  symptr->modptr->archentry);
				continue;
			}
...
bin_off_t value;
{
	...
	if (symptr->modptr->class != (class = modlast->class))
		for (rlptr = redfirst;; rlptr = rlptr->rlnext) {
			...
			if (symptr == rlptr->rlsymptr
			    && class == rlptr->rlmodptr->class)
				break;
		}
	return TRUE;
}
...
char *sizedesc;
{
	return 3 & ((unsigned)sizedesc[((NSEG - 1) - seg) / 4] >>
		    (2 * (seg % 4)));
