...
bool_pt argxsym;
{
	...
	for (modptr = modfirst; modptr != NUL_PTR; modptr = modptr->modnext)
		if (modptr->loadflag) {
			...
			for (symparray = modptr->symparray;
				...
				if (symptr->modptr == modptr
					...
					if (!
						...
						tempoffset =
						    ld_roundup(symptr->value, 4,
							       bin_off_t);
						/* temp kludge quad alignment for 386 */
						symptr->value = comsz[seg =
								      symptr->
								      flags &
								      SEGM_MASK];
			...
			for (seg = 0, cptr = modptr->segsize; seg < NSEG; ++seg) {
				...
				segsz[seg] =
				    ld_roundup(segsz[seg], 4, bin_off_t);
				comsz[seg] =
				    ld_roundup(comsz[seg], 4, bin_off_t);
				cptr += sizecount;
			}
		}
	...
	for (seg = 0; seg < NSEG; ++seg) {
		...
		if (seg > 3) {
			...
			segboundary[7] = 'O';
			setsym(segboundary,
			       (bin_off_t) (segadj[seg] - segadj[0]) / 0x10);
			/* __segXSO */
		}
#endif
	}
	...
	if (!stripflag) {
		...
		for (modptr = modfirst; modptr != NUL_PTR;
		     modptr = modptr->modnext)
			if (modptr->loadflag) {
				register struct symstruct **symparray;
				register struct symstruct *symptr;

				for (symparray = modptr->symparray;
				     (symptr = *symparray) != NUL_PTR;
				     ++symparray)
					if (symptr->modptr == modptr) {
#if ELF_SYMS
						if (symptr->name[0] == '_'
...
#else
						strncpy((char *)extsym.n_name,
							symptr->name,
							sizeof extsym.n_name);
#endif
						u4cn((char *)&extsym.n_value,
						     (u4_t) symptr->value,
...
#else
				fatalerror
				    ("relocation by long offsets not implemented");
#endif
			case CM_1_SKIP:
				skip(1);
				break;
		...
		case CM_OFFSET_RELOC:
			...
			if (modify & R_MASK) {
				...
				if (curseg != m && m != SEGM_MASK)
					interseg(modptr->filename,
						 modptr->archentry, (char *)0);
#endif
				offset -= (spos + relocsize);
			}
			offtocn(buf, segbase[modify & SEGM_MASK] + offset,
				relocsize);
...
struct modstruct *modptr;
{
	...
	for (seg = 0, sizeptr = modptr->segsize; seg < NSEG; ++seg) {
		size = cntooffset(sizeptr,
				  sizecount =
				  segsizecount((unsigned)seg, modptr));
		sizeptr += sizecount;
		...
		if ((size =
		     ld_roundup(segpos[seg], 4,
				bin_off_t) - segpos[seg]) != 0) {
			setseg(seg);
			writenulls(size);
			segpos[seg] = spos;
		}
